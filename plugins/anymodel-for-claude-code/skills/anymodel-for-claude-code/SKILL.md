...
description: ModelHub —— Claude Code 多模型网关，本地零依赖，让 Claude Code 通过统一的 OpenAI 兼容端点路由到任意模型（Claude / DeepSeek / SiliconFlow / OpenRouter / 智谱 GLM / Kimi / Gemini / Qwen / Groq 等 8 家 21 个模型）。本地 Node 代理做 Anthropic↔OpenAI 协议转换，tool use 双向转换，管理界面一键热切换，API Key 本地落盘，Windows 友好。Claude 本身是一等路由，按需为不同任务选最合适模型。触发词：Claude Code 多模型网关、multi-model gateway、model router、Claude Code 代理、local llm gateway、ModelHub、AnyModel、route Claude Code to any model
---

# ModelHub — Claude Code 多模型网关

> 前身为 AnyModel for Claude Code，现统一为 **ModelHub** 品牌。安装名 `anymodel-for-claude-code` 保持不变（向后兼容）。
...
- **密钥本地落盘**：可通过界面把 Key 存到 `~/.modelhub/keys.json`（也可继续用环境变量），不硬编码进代码
- **选择持久化**：界面切换的模型写入 `~/.modelhub/state.json`，**代理重启后仍生效**，不用每次重选
...
- **方式 A — 界面填写（推荐）**：启动后打开界面，在「配置 API 密钥」卡片里逐家填入，保存即写入 `~/.modelhub/keys.json`（密钥仅存本机，不上传）。
...
4. **② 配置 API 密钥**：逐家填入并保存（写 `~/.modelhub/keys.json`）
...
| POST | `/api/switch` | 切换当前模型 `{ "model": "deepseek-chat" }`（写入 `~/.modelhub/state.json` 持久化） |
...
| 500 "缺少 API key" | Key 未配 | 在界面「配置 API 密钥」填写，或在环境变量 / `~/.modelhub/keys.json` 设好 |
| 界面点切换没反应 | 代理未运行 | 先启动 `proxy.js`，界面会提示「无法连接代理」 |

## 📤 发布到 CodeBuddy 插件市场

本仓库已按 CodeBuddy 插件市场规范组织：根目录含 `.codebuddy-plugin/marketplace.json`（市场清单），插件位于 `plugins/anymodel-for-claude-code/`（含 `.codebuddy-plugin/plugin.json` + `skills/anymodel-for-claude-code/`）。仓库同时包含 npm CLI 形态（`bin/` + `lib/` + `assets/`），可 `npm install -g modelhub` 直接使用。

发布流程：

1. **推送到 GitHub 公开仓库** `wind33441998/modelhub`（旧地址 `wind33441998/anymodel-for-claude-code` 会自动重定向）
2. **他人在 CodeBuddy 内添加市场并安装**：
   ...
   ```
   > 也可用完整 URL：`/plugin marketplace add https://github.com/wind33441998/modelhub`
3. **变现**：CodeBuddy 官方 marketplace 为开源协作模式，若需商业化分发，可在仓库 README 引导赞助/付费授权；付费渠道另走 SkillHQ / SkillStack

## Resources

### scripts/
- **proxy.js**：Node 原生实现的 Anthropic↔OpenAI 协议代理，零依赖。支持流式响应、tool_use 双向转换、多 provider 路由、`default`/`auto` 热切换、模型选择持久化（`~/.modelhub/state.json`）、活动日志、内置 `echo` 自检、`/api/logs` 与 `/api/selftest` 管理 API、仅监听 `127.0.0.1`。
- **config.json**：8 家供应商配置，API key 用 `${ENV}` 占位（运行时从环境变量读取），也可由界面写 `~/.modelhub/keys.json` 覆盖。
